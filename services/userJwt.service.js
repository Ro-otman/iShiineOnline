...
const ACCESS_TOKEN_ISSUER = 'ishiine-online-mobile';
const ACCESS_TOKEN_AUDIENCE = 'ishiine-mobile-user';
...
function getAccessTokenSecret() {
  const secret = String(env.USER_ACCESS_TOKEN_SECRET || '').trim();
  if (!secret) {
    throw buildAuthError(
      'Configurer USER_ACCESS_TOKEN_SECRET pour activer la session mobile.',
      503,
      'USER_ACCESS_SECRET_MISSING',
    );
  }
  return secret;
}
...
export function signUserAccessToken(user) {
  ...
  const ttlInSeconds = Math.max(
    60 * 60,
    Number(env.USER_ACCESS_TOKEN_TTL_DAYS || 180) * 24 * 60 * 60,
  );
  ...
  const payload = {
    ...
    sub: String(user.id_users || user.idUser || ''),
    type: 'user_access',
    iat: nowInSeconds,
...
export function verifyUserAccessToken(token) {
  ...
  if (header?.alg !== 'HS256' || payload?.type !== 'user_access') {
    throw buildAuthError(
      'Session utilisateur invalide.',
      401,
      'USER_ACCESS_TOKEN_INVALID',
  ...
  const idUser = String(payload.sub || '').trim();
  if (!idUser) {
    throw buildAuthError(
      'Session utilisateur invalide.',
      401,
